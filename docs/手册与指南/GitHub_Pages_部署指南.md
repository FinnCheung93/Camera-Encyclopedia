...
> 过期时间：可按需选择 30/60/90 天或更长；到期后需重新生成 Token，并在站点上的 **`setup-token.html`** 重新保存（勿把 Token 写进仓库内的 `config.js`）。
...
4. 可选：在后台「网站基础配置」把 `data.json` 里的 `siteConfig.githubConfig` 填成与 `config.js` 相同的 owner/repo/branch（**仅备忘**，不参与 API；避免以后只看 `data.json` 产生误解）。

> 仅个人使用、且仓库为 **Private** 时，若你仍想把 Token 写在 `config.js` 里，也可能被 Secret scanning 拦截；**推荐始终使用 `setup-token.html`**。

### 4.1 让别人能打开前台（不给他 Token）

- 把仓库设为 **Public**（Settings → Danger zone / 或创建时选 Public）。
- 当前站点会**先尝试匿名**读取 `data.json`，公开仓库下**访客浏览器不需要 PAT**。
- **后台保存**仍只有你本机配置了 PAT 才能写回仓库；匿名访问不能代替写入权限。
- 匿名调用 GitHub API 有 **每小时约 60 次/ IP** 的额度，个人小站一般足够。
...
- **保存失败 401/403**：PAT 权限不足或过期；重新生成并在 **`setup-token.html`** 保存。若提示 **sha / 版本冲突**：在其他窗口保存过 `data.json` 时常见，**刷新后台页**再编辑保存。
